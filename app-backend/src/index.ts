  ...
  try {
    ...
    if (localProp) {
      ...
      const finalAssets = {
        ...
        specTable: {
          // Only the fields the agent actually produces are pre-filled
          // (materials / mechanism); the rest are left blank for the art
          // department to fill in from the dossier — no fabricated demo values.
          dimensionsClosed: '',
          dimensionsOpen: '',
          weight: '',
          materials: agentProp.final_assets?.material_spec || '',
          finishes: '',
          stuntVariant: '',
          scriptedStates: '',
          mechanism: agentProp.final_assets?.build_spec || '',
          caregivingNotes: '',
        },
